...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .desc {{ background:#fff; border:1px solid var(--rule); border-radius:12px; padding:12px 16px; box-shadow:0 1px 0 rgba(15,20,25,.03); margin-bottom:14px; }}
  .desc h3 {{ font-family:var(--mono); font-size:11px; letter-spacing:.14em; text-transform:uppercase; color:var(--steel); margin:0 0 8px; }}
  .descrows {{ display:flex; flex-wrap:wrap; gap:6px 18px; }}
  .row {{ font-size:13.5px; }} .row .k {{ font-family:var(--mono); font-size:11px; color:var(--steel); margin-right:5px; }}
...
<body><div class="wrap">
  ...
  <p class="lead">A use case diagram says <em>what</em> the system does; an <strong>activity diagram</strong> shows <em>how one use case flows</em>, in what order, with which decisions, and who does each step. Here we take the <strong>Book room</strong> use case and unpack its description into a flow.</p>
