...
def builder(bid, svg, vb, narr, title, sub):
  ...
  <div class="builder" id="{bid}">
    <div class="controls">
      ...
      <button class="breset secondary">Reset</button><button class="bplay secondary">&#9658; Play</button><button class="bfs secondary">&#9974; Full screen</button>
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .controls {{ display:flex; align-items:center; gap:10px; flex-wrap:wrap; position:sticky; top:0; background:#fff; z-index:5; padding:6px 0 8px; border-bottom:1px solid var(--rule); }}
...
function makeBuilder(root){{
  ...
  function render(){{
    ...
    if(cur>0){{ const cu=el('.bstep.current'); if(cu&&cu.scrollIntoView) cu.scrollIntoView({{behavior:'smooth',block:'center'}}); }}
  }}
  ...
  el('.bplay').onclick=()=>{{ if(timer){{stop();return;}} if(cur===MAX)cur=0; if(!document.fullscreenElement&&root.requestFullscreen) root.requestFullscreen(); el('.bplay').textContent='Pause'; timer=setInterval(()=>{{ if(cur>=MAX){{stop();return;}} cur++; render(); }}, 2100); }};
  const fsb=el('.bfs'); if(fsb) fsb.onclick=()=>{{ if(document.fullscreenElement){{document.exitFullscreen();}} else if(root.requestFullscreen){{root.requestFullscreen();}} }};
  render();
}}
