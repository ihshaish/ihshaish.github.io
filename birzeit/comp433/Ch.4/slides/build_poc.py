# POC v3: animated use case diagram build, derived visibly from requirements.
...
def actor(x,y,name,sub=""):
    s=f'<g transform="translate({x},{y})"><circle cx="20" cy="6" r="8" class="actor-blue"/>'
...
S=['<defs><marker id="ucOpen" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="11" markerHeight="11" orient="auto"><path d="M 0 0 L 10 5 L 0 10" fill="none" stroke="#2c3540" stroke-width="1.4"/></marker><marker id="assoc" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M 0 0 L 10 5 L 0 10 Z" fill="#4a8bb8"/></marker></defs>']
S.append(g(1,'<rect x="250" y="70" width="740" height="600" rx="8" class="uml-box" fill="#faf7f2" stroke-width="1.8"/>','<text x="620" y="60" text-anchor="middle" class="uml-text" font-weight="600" font-size="13">Hotel Booking System</text>'))
S.append(g(2, actor(60,250,"Guest","")))
...
inc_chip='<code>&lt;&lt;include&gt;&gt;</code>'; ext_chip='<code>&lt;&lt;extend&gt;&gt;</code>'
NARR=[
 "A use case diagram is built straight from the requirements. As each part appears, its source in the narrative or the UR list lights up. Use Next, or press Play.",
 "Start with the system boundary. Everything inside it is the system we build; everything outside it is an actor that interacts with the system.",
 "Choosing actors: scan the requirements for the people, roles and outside systems that interact with the system. A guest appears throughout, so Guest is our first actor, a primary actor, because it uses the system to reach a goal of its own.",
...
MAX=len(NARR)-1
HL={2:['nar-guest','ur1'],3:['nar-search','ur1'],4:['nar-book','ur2'],5:['nar-cancel','ur3'],6:['nar-manager','ur4'],7:['nar-avail'],10:['nar-pay','ur2'],12:['nar-discount','ur5'],14:['nar-pay','ur2']}
import json
HTML=f'''<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Use case diagram: animated build</title>
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .bhead {{ position:sticky; top:0; z-index:5; background:#fff; padding:6px 0 8px; border-bottom:1px solid var(--rule); margin-bottom:4px; }}
  ...
  .bstep {{ opacity:0; transform:translateY(12px) scale(.9); transform-box:fill-box; transform-origin:center; transition:opacity .32s ease, transform .44s cubic-bezier(.2,.8,.3,1), filter .35s ease; }}
  .bstep.on {{ opacity:1; transform:translateY(0) scale(1); }}
  .bstep.current {{ filter:drop-shadow(0 0 7px rgba(184,92,56,.55)); }}
  .bstep.draw {{ transform:none; transition:opacity .2s ease, filter .35s ease; }}
  .drawn {{ stroke-dasharray:320; stroke-dashoffset:320; }}
  .bstep.draw.on .drawn {{ stroke-dashoffset:0; transition:stroke-dashoffset .85s ease .12s; }}
</style></head>
<body><div class="wrap">
...
open("poc_usecase_build.html","w",encoding="utf-8").write(HTML)
print("wrote v3; steps:", MAX)
